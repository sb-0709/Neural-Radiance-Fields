...
def load_data(data_path: str):
    ...
    Handles two formats:
      - Lego format:  images_train, images_val, c2ws_train, c2ws_val, c2ws_test, focal
      - Custom COLMAP format: images_train, c2ws_train, focal only
        -> auto-splits last 5 images as val, generates 60 circular test cameras

    Returns:
        images_train: numpy array (num_train, H, W, 3) in [0,1]
        c2ws_train:   numpy array (num_train, 4, 4)
        images_val:   numpy array (num_val, H, W, 3) in [0,1]
        c2ws_val:     numpy array (num_val, 4, 4)
        c2ws_test:    numpy array (num_test, 4, 4)
    ...
    # Lego-format: val and test splits already present
    if "images_val" in data:
        images_val = data["images_val"] / 255.0
        c2ws_val   = data["c2ws_val"].astype(np.float32)
        c2ws_test  = data["c2ws_test"].astype(np.float32)
        print(f"Lego dataset: {images_train.shape[0]} train | "
              f"{images_val.shape[0]} val | {c2ws_test.shape[0]} test cams")

    # Custom COLMAP format: create splits automatically
    else:
        print("Custom dataset: auto-creating val/test splits...")
        total   = images_train.shape[0]
        num_val = min(5, max(1, total // 10))

        images_val   = images_train[-num_val:].copy()
        c2ws_val     = c2ws_train[-num_val:].copy()
        images_train = images_train[:-num_val]
        c2ws_train   = c2ws_train[:-num_val]

        # Build 60 circular test cameras at the average training camera radius
        positions = c2ws_train[:, :3, 3]          # (N, 3)
        radius    = float(np.mean(np.linalg.norm(positions, axis=1)))
        avg_z     = float(np.mean(positions[:, 2]))
        print(f"  orbit radius={radius:.3f}  avg_z={avg_z:.3f}")

        c2ws_test = []
        for phi in np.linspace(0, 2 * np.pi, 60, endpoint=False):
            pos     = np.array([radius * np.cos(phi),
                                radius * np.sin(phi),
                                avg_z], dtype=np.float32)
        ...
        c2ws_test = np.stack(c2ws_test)

        print(f"  {images_train.shape[0]} train | {images_val.shape[0]} val | "
              f"{c2ws_test.shape[0]} test cams (auto orbit)")
